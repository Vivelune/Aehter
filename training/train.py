...
SM_CHANNEL_TEST = os.environ("SM_CHANNEL_TEST", "/opt/ml/input/data/testing")
...
def main():
    ...
    if not install_ffmpeg():
        print("Error: FFMPEG installion failed. Cannot continue training.")
        sys.exit(1)
    ...
    for epoch in tqdm(range(args.epochs), desc="Epochs"):
        ...
        if val_loss["total"] < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(args.model_dir, "model_pth"))
            
    # AFTER TRAINING IS COMPLETE AND EVALUATE ON TEST SET
